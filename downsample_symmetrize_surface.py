...
def symmetrize_c2v(df, keep_both=True, missing_value=-8.0, tol=1e-8):
    """
    C2v symmetry: average over the orbit {(x,y), (-x,y), (-x,-y), (x,-y)}.
    - If multiple points exist in the orbit, use their mean z.
    - If only one exists, copy that z to all members (or to the representative if keep_both=False).
    - If none (hole), mark as missing_value.
    Output is sorted by y desc, then x asc, with a 'status' column.
    """
    df = df.copy()
    df['x_r'] = df['x'].round(4)
    df['y_r'] = df['y'].round(4)
    ...
    for xr, yr, z in zip(df['x_r'], df['y_r'], df['z']):
        ...
        # Full C2v orbit (dedup automatically if xr or yr is 0.0)
        orbit = {( xr,  yr),
                 (-xr,  yr),
                 (-xr, -yr),
                 ( xr, -yr)}
        ...
        z_vals = []
        for k in orbit:
            zv = z_map.get(k, missing_value)
            if not (abs(zv - missing_value) <= tol):
                z_vals.append(zv)

        if len(z_vals) == 0:
            z_avg, status = missing_value, 'missing'
        elif len(z_vals) == 1:
            z_avg, status = z_vals[0], 'single'
        else:
            z_avg, status = float(np.mean(z_vals)), 'average'

        if keep_both:
            # Write all unique orbit members with the same averaged z
            for (xk, yk) in orbit:
                rows.append((xk, yk, z_avg, status))
        else:
            # Keep a single representative (the current key)
            rows.append((xr, yr, z_avg, status))
...
def process_single_file(filepath, keep_both, steps, deltag_path, symm):
    ...
    def apply_symm(df_in):
        if symm == "c1":
            ...
            return symmetrize_c2v(df_in, keep_both=keep_both) 
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--symm', choices=['c1', 'c2', 'c2v', 'csx', 'csy'], required=True,
                        help=("c1 : leave map points unchanged\n"
                              "c2 : inversion symmetry, average (x,y) with (-x,-y)\n"
                              "c2v : average over (x,y), (-x,y), (-x,-y), (x,-y)\n"
